...
%% Reading image
im = imread(['Treasure_easy' ...
    '.jpg']); % change name to process other images
figure;
imshow(im);
...
while go
     current_object_direction=direction(props(current_object).Centroid,props(current_object).Cetroid_of_yellow_dot);
      ...
      if props(current_object).arrow_id ~= 'Not_arrow'
         text(props(current_object).Centroid(1),props(current_object).Centroid(2)-40,sprintf("%d",props(current_object).arrow_id),Color='r');
      end
